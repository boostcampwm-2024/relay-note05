...
- A sound mind in a sound body<br>
  : 매일 10분이상 산책하고, 여기에 기록 올리기 ex) 사진 또는 어플기록
...
- 오늘 하루 날씨 알림 요정이 되어보아요<br>
  : 매일 아침, 슬랙에 오늘의 날씨를 알려주세요. 날씨 요정이 되어 분위기를 밝게 만들어 보아요!

  예? 제가요? (어이없음)

- 변경사유 : (기존) 다음 릴프시간 카페에서 책읽기 -> 다음 릴프가 없으므로 변경
...
> "많이 읽고, 많이 쓰고, 많이 생각하자"
> 
- 코드는 글쓰기 같으며, 갈수록 코드를 읽는 능력이 중요해지고 코드를 잘 쓰려면 많이 읽어야한고 합니다. 우리가 왜 남의 코드를 읽는 연습을 계속 하고 있는지 느낄 수 있었고, 스스로 피어세션 전 팀원들의 코드를 읽는게 어렵다고 느꼈기 때문에 여기서 나온 코드를 잘 읽는 방법(전체를 빠르게 읽고 반복해서 읽어라)을 스스로 적용해봐야겠다 생각했습니다.

### <개발자 원칙> - Chatper 4

> “낯선 방식으로 해결” “과정을 기록하기” “해부하지 말고 직접 만들기”
> 
- 전반적으로 우리가 진행하는 챌린지 과정이 어떤 기반 생각을 가지고 만들어졌는지 이해할 수 있었고 우리가 챌린지를 진행하며 어떤 마음가짐으로 임해야할지 생각해보는 좋은 기회가 됐다고 생각합니다.
- 남에게 설명하는 것이 이미 알고 있는 것과 알고 있다고 착각하는 지식을 구분하는데 도움이 된다는데, 이걸 주간학습퀴즈에서 비디오를 녹화하며 느꼈습니다.
- git, 메모리 시뮬레이터 등을 만들어보면서 무언가에 대해 똑같이 동작하도록 만드는게 지식을 체화하고 명확히 이해에 도움이 된다는 것도 느낄 수 있었습니다.
- 과정을 기록하는 것도 매일 gist에 md파일을 작성하며 하고 있지만 여전히 어떤 방식이 옳은지 모르겠습니다. 하지만 이런 기록을 왜 해야하는지를 이 단락을 읽으며 이해할 수 있었고 또한 3,4주에 진행하는 짝 해결과 드라이버,네이게이터 역할을 수행하는것의 목적이 무엇인지 그리고 어떻게 역할을 수행해야 좋은 배움을 얻어갈 수 있을지도 생각해볼 수 있었습니다.
...
- 미션을 해결하면서 구현을 할 때, 일단 돌아가는 코드와 깔끔하게 만들어진 코드 중 어떤게 더 좋을지에 대해 고민했었고 지금도 고민중입니다. 여기서는 일단 동작하는 것이 더 중요하다고 합니다. 회사의 납품 기한까지 완성하지 못하면 아무리 깔끔한 코드라고 해도 의미가 없다는 의미에서 그런 것인데, 3주차 개선하기를 진행하면서 일단 만들고 개선하는 것이 더 좋다고 생각해 어느정도 공감을 했습니다.

> The good thing about reinventing the wheel is that you can get a round one.

- 일단 기존에 있는 것을 다시 만들어보는 건 결과가 어떻든 그것을 이해하는데 도움이 된다고 합니다. 미션을 해결하면서 어설프게 돌아가게 구현을 해도 그것을 구현하기까지 원리를 이해하려고 계속 학습을 하고 정리를 했기 때문에 조금이라도 더 잘 알게되었다고 생각해서 인상이 깊었습니다.
